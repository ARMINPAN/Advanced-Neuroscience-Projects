...
t0 = 0:0.002:0.010; % refractory periods
...
CV = zeros(trials,length(mag),length(tau_peak));

figure;
for ii = 1:trials
for j = 1:size(CV,2) % magnitude of the EPSCs
    for jj = 1:size(CV,3) % tau_peak of the EPSCs
        [ii j jj]
        Kernel = t .* exp(-t/tau_peak(jj)); 
        v = vr * ones(1,T); % Vector of output voltage
        R = 1;
        dv = 0;
        flag = 0;
        ...
        train_downS(ksdown) = 1;
        timeSpikes = downsample(timeSpikes,k);
        I = mag(j)*(conv(Kernel,train_downS))/max(abs(conv(Kernel,train_downS)));
        i = 1;
        % Euler method for v(t)
        while i <= (T-1)
            % Spike
        ...
        % cal CVs
        [peaks, spkess] = findpeaks((v),1/dt);
        spkes = spkess(find(v(int32(spkess/dt)) >= vth))/dt;
        CV(ii,j,jj) = std(diff(spkes))/mean(diff(spkes));
    end
end
end

CV = reshape(mean(CV,1),[size(CV,2) size(CV,3)]);

figure;
subplot(1,2,1)
for i = size(CV,1):-10:size(CV,1)-30
    plot(tau_peak,CV(i,:),'LineWidth',2);
    grid on; grid minor;
    hold on;
    xlabel('tau_peak','interpreter','latex');
    ylabel('CV','interpreter','latex');
    title('CV vs tau_peak of EPSC','interpreter','latex');
end
legend("EPSC Amp = " + mag(end),"EPSC Amp = " + mag(end-10),...
    "EPSC Amp = " + mag(end-20),"EPSC Amp = " + mag(end-30))

subplot(1,2,2)
for i = size(CV,2):-1:size(CV,2)-3
    plot(mag,CV(:,i),'LineWidth',2);
    grid on; grid minor;
    hold on;
    xlabel('magnitude','interpreter','latex');
    ylabel('CV','interpreter','latex');
    title('CV vs Magnitude of EPSC','interpreter','latex');
end
legend("EPSC Width = " + tau_peak(end),"EPSC Width = " + tau_peak(end-1),...
    "EPSC Width = " + tau_peak(end-2),"EPSC Width = " + tau_peak(end-3))
...
for j=1:450% 450 currents for inhibitory and excitatory
    ...
    ksdown = (downsample(find(vector(j,:) == 1),1));
